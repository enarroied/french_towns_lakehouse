...
def upload_scraper_output(
    ...
    if _should_skip_file(base_name, md5, known_hashes):
        log(f"⏭️ Skipping {scraper_name} — hash unchanged")
        csv_path.unlink()
        return None
...
def _extract_file(
    ...
    try:
        ...
        log(f"✅ Extracted to {output_dir}", logger=logger)
        for name in zip_ref.namelist():
            extracted_files.append(output_dir / name)
        file_path.unlink()
        log(f"🗑️ Removed {file_path}", logger=logger)
    except zipfile.BadZipFile:
        dest_path = output_dir / file_path.name
        shutil.move(str(file_path), str(dest_path))
        log(f"✅ Moved to {dest_path}", logger=logger)
...
def _upload_file(
    ...
    log(f"☁️ Uploaded {key} to {staging_bucket}", logger=logger)
    file_path.unlink()
...
def _process_extracted_files(
    ...
    for extracted_file in extracted_files:
        ...
        if _should_skip_file(base_name, md5, known_hashes):
            log(f"⏭️ Skipping {base_name} — hash unchanged", logger=logger)
...
async def _download_and_upload(
    ...
    if url is None:
        log("⚠️ No URL provided for download item, skipping", logger=logger)
        return []
    ...
    async with semaphore:
        try:
            ...
            log(f"❌ HTTP error downloading {filename}: {e}", "error", logger=logger)
            return []
        except Exception as e:
            log(f"❌ Failed to download {filename}: {e}", "error", logger=logger)
            return []
        finally:
