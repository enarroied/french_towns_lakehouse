...
import pytest
from flows_staging.shared.audit import _check_db
from flows_staging.shared.audit import _check_internet_connection
from flows_staging.shared.audit import _check_minio
from flows_staging.shared.audit import _write_file_metadata
from flows_staging.shared.audit import get_latest_hashes
from flows_staging.shared.audit_db import _update_latest_run
from flows_staging.shared.audit_db import _update_run_status
...
class TestGetLatestHashes:
    ...
    def test_returns_empty_dict_when_no_hashes(self):
        """Should return empty dict when no hashes exist."""
        pytest.skip("Requires Prefect API at http://127.0.0.1:4200")

        with patch("flows_staging.shared.audit_db.get_latest_hashes", return_value={}):
...
class TestWriteFileMetadata:
    ...
    def test_delegates_to_audit_db(self):
        """Should call audit_db.write_file_metadata with correct args."""
        ...
        with patch(
            "flows_staging.shared.audit.audit_db.write_file_metadata"
        ) as mock_fn:
            _write_file_metadata(config, record, now)

        mock_fn.assert_called_once_with(config, record, now)


class TestUpdateLatestRun:
    """Tests for _update_latest_run function."""

    def test_marks_previous_runs_as_not_latest(self):
        """Should set is_latest=0 for previous runs and is_latest=1 for the new run."""
        mock_cursor = MagicMock()
        mock_conn = MagicMock()
        mock_conn.cursor.return_value.__enter__.return_value = mock_cursor

        _update_latest_run(mock_conn, "new-run-id", "demographics")

        assert mock_cursor.execute.call_count == 2
        call_args_1 = mock_cursor.execute.call_args_list[0][0]
        call_args_2 = mock_cursor.execute.call_args_list[1][0]
        assert "UPDATE" in call_args_1[0]
        assert "demographics" in call_args_1[1]
        assert "new-run-id" in call_args_1[1]
        assert "UPDATE" in call_args_2[0]
...
class TestUpdateRunStatus:
    ...
    def test_updates_status_and_end_time(self):
...
class TestCheckDB:
    ...
    def test_calls_migrate(self):
        """Should call audit_db.migrate."""
        with (
            patch("flows_staging.shared.audit.audit_db.migrate") as mock_migrate,
    ...
    def test_raises_on_migrate_failure(self):
        ...
        with (
            patch(
                "flows_staging.shared.audit.audit_db.migrate",
                side_effect=Exception("DB Error"),
            ),
            patch("flows.shared.logging.get_run_logger") as mock_logger,
        ):
            mock_logger.return_value = MagicMock()
            with pytest.raises(RuntimeError, match="Metadata DB not reachable"):
                _check_db()
