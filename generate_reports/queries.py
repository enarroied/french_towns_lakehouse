...
def get_departments(conn: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    query = """
        SELECT DISTINCT department_code, department_name
        FROM silver.dim_communes_france
...
def get_city_data(
    ...
    return conn.execute(query, [department_code, department_code]).fetchdf()


def get_population_timeseries(
    conn: duckdb.DuckDBPyConnection, commune_id: str
) -> pd.DataFrame:
    query = """
        ...
        ORDER BY year
    """
    return conn.execute(query, [commune_id]).fetchdf()
