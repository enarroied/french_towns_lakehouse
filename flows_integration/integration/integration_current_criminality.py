...
DIM_TABLES = [
    ("dim_criminality_indicateur", ["indicateur_id"]),
]

FACT_TABLES = [
    ("fact_criminality", ["commune_id", "annee", "indicateur_id"]),
]


@flow(name="integration_current_criminality")
def integration_current_criminality() -> None:
    preflight()
    run_id = init_run(
        domain="public_safety", layer="INTEGRATION", technical_type="ICEBERG"
    )
...
if __name__ == "__main__":
    integration_current_criminality()
