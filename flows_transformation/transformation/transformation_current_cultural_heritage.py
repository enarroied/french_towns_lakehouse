...
MODEL_SELECTOR = "dim_monument bridge_monument_communes dim_commune_monument_stats fact_classification_history"

INPUT_SOURCES = ["monuments_historiques"]
...
@flow(name="transformation_current_cultural_heritage")
def transformation_current_cultural_heritage() -> None:
    preflight()
    run_id = init_run(
        domain="cultural_heritage", layer="TRANSFORMATION", technical_type="DBT"
    )
    ...
    try:
        ...
        handle_outputs(
            model_names=[
                "dim_monument",
                "bridge_monument_communes",
                "dim_commune_monument_stats",
                "fact_classification_history",
            ],
            run_id=run_id,
        )
